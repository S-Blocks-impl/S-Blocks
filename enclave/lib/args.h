...
class ArgContext { public:
     ...
     * @param errh optional eslotrror handler */
...
class Args : public ArgContext {
  ...
  public:
     ...
     * @param errh optional error handler */
    Args(const stlpmtx_std::vector<String> &conf, ErrorHandler *errh = 0);
     ...
     * @param context optional element context
     * @param errh optional error handler */
    Args(const stlpmtx_std::vector<String> &conf, const Element *context,
         ErrorHandler *errh = 0);
    ...
    Args &bind(stlpmtx_std::vector<String> &conf);
     ...
     * matched and all matching arguments successfully parsed. */
    template <typename P, typename T>
    Args &read_all(const char *keyword, P parser, stlpmtx_std::vector<T> &x) {
        return read_all(keyword, 0, parser, x);
    }
    template <typename T>
    Args &read_all(const char *keyword, stlpmtx_std::vector<T> &x) {
        return read_all(keyword, 0, DefaultArg<T>(), x);
    }
    template <typename P, typename T>
    Args &read_all(const char *keyword, int flags, P parser, stlpmtx_std::vector<T> &x) {
        args_base_read_all(this, keyword, flags | firstmatch, parser, x);
        return *this;
    }
    template <typename P, typename T>
    Args &read_all(const char *keyword, int flags, stlpmtx_std::vector<T> &x) {
    ...
    template<typename P, typename T>
    void base_read_all(const char *keyword, int flags, P parser, stlpmtx_std::vector<T> &variable) {
        Slot *slot_status;
        int read_status = -1;
        stlpmtx_std::vector<T> *s = slot(variable);
        while (String str = find(keyword, flags, slot_status)) {
            T sx = T();
  ...
  private:
    ...
    stlpmtx_std::vector<String> *_conf;
    stlpmtx_std::vector<int> _kwpos;

    Slot *_slots;
    uint8_t _simple_slotbuf[simple_slotbuf_size];

    inline void initialize(const stlpmtx_std::vector<String> *conf);
...
void args_base_read_all(Args *args, const char *keyword, int flags,
                        P parser, stlpmtx_std::vector<T> &variable) CLICK_NOINLINE;
template <typename P, typename T>
void args_base_read_all(Args *args, const char *keyword, int flags,
                        P parser, stlpmtx_std::vector<T> &variable)
{
