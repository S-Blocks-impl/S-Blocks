...
class ElementTracker : public RouterVisitor { public:
    ...
    ElementTracker(Manager_enclave *router);

    /** @brief Return the elements that matched. */
    const stlpmtx_std::vector<Element *> &elements() const {
	return _elements;
    }
    ...
    /** @brief Iterator type. */
    typedef stlpmtx_std::vector<Element *>::const_iterator const_iterator;
...
class ElementCastTracker : public ElementTracker { public:
    ...
    ElementCastTracker(Manager_enclave *router, const String &name)
...
class ElementNeighborhoodTracker : public ElementTracker { public:
     ...
     * traverse) */
    ElementNeighborhoodTracker(Manager_enclave *router, int diameter = 1)
