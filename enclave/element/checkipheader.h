...
class CheckIPHeader : public BatchElement { public:
  ...
  int configure(stlpmtx_std::vector<String> &, ErrorHandler *) CLICK_COLD;
  ...
  struct OldBadSrcArg {
      static bool parse(const String &str, stlpmtx_std::vector<IPAddress> &result,
            Args &args);
  };

  struct InterfacesArg {
      static bool parse(const String &str, stlpmtx_std::vector<IPAddress> &result_bad_src,
            stlpmtx_std::vector<IPAddress> &result_good_dst, Args &args);
  };
 ...
 private:
  ...
  stlpmtx_std::vector<IPAddress> _bad_src;   // array of illegal IP src addresses
  ...
  stlpmtx_std::vector<IPAddress> _good_dst;  // array of IP dst addrs for which _bad_src
                // does not apply
