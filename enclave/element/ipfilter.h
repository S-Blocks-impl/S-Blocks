...
class IPFilter : public BatchElement { public:
	...
	int check(const Primitive &prev_prim, int level,
		  ...
		  ErrorHandler *errh);
	void compile(Classification::Wordwise::Program &p, stlpmtx_std::vector<int> &tree) const;
	...
	void add_comparison_exprs(Classification::Wordwise::Program &p, stlpmtx_std::vector<int> &tree, int offset, int shift, bool swapped, bool op_negate) const;
  ...
  private:
    ...
    struct Parser {
	const stlpmtx_std::vector<String> &_words;
	stlpmtx_std::vector<int> &_tree;
	Classification::Wordwise::Program &_prog;
	const Element *_context;
	ErrorHandler *_errh;
	Primitive _prev_prim;

	Parser(const stlpmtx_std::vector<String> &words, stlpmtx_std::vector<int> &tree,
