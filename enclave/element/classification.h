...
void add_insn(stlpmtx_std::vector<int> &tree, int offset, uint32_t value, uint32_t mask);

    stlpmtx_std::vector<int> init_subtree() const;
    void start_subtree(stlpmtx_std::vector<int> &tree) const;
     ...
     *   match. */
    void negate_subtree(stlpmtx_std::vector<int> &tree, bool flip_short = false);
    inline void finish_subtree(stlpmtx_std::vector<int>& tree, Combiner op) {
        finish_subtree(tree, op, j_success, j_failure);
    }
    void finish_subtree(stlpmtx_std::vector<int>& tree, Combiner op, int success, int failure);
    ...
    void count_inbranches(stlpmtx_std::vector<int> &inbranches) const;
...
class CompressedProgram { public:
  ...
  private:
    ...
    stlpmtx_std::vector<uint32_t> _zprog;
    int _output_everything;
    unsigned _safe_length;
    unsigned _align_offset;
...
class DominatorOptimizer { public:
  ...
  private:
    ...
    Program *_p;
    stlpmtx_std::vector<int> _known_length;
    stlpmtx_std::vector<int> _insn_id;
    stlpmtx_std::vector<int> _dom;
    stlpmtx_std::vector<int> _dom_start;
    stlpmtx_std::vector<int> _domlist_start;
#if CLICK_CLASSIFICATION_WORDWISE_DOMINATOR_FASTPRED
    mutable stlpmtx_std::vector<int> _pred_first;	// indexed by state (insn id)
    mutable stlpmtx_std::vector<int> _pred_next;	// indexed by branch
    mutable stlpmtx_std::vector<int> _pred_prev;	// indexed by branch
#endif
    ...
    static void intersect_lists(const stlpmtx_std::vector<int> &, const stlpmtx_std::vector<int> &, const stlpmtx_std::vector<int> &, int pos1, int pos2, stlpmtx_std::vector<int> &);
    static int last_common_state_in_lists(const stlpmtx_std::vector<int> &, const stlpmtx_std::vector<int> &, const stlpmtx_std::vector<int> &);
    void find_predecessors(int state, stlpmtx_std::vector<int> &) const;
    int dom_shift_branch(int brno, int to_state, int dom, int dom_end, stlpmtx_std::vector<int> *collector);
